...
import static GUI.product_section.stock_section.add_stock.log;
...
public class update_stock extends javax.swing.JFrame {
    static Logger log=Logger.getLogger(update_stock.class.getName());
...
stockModel adm;
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        if(id.equals("") && name.equals("") && stocks.equals("")){
            String result="stock updating error";
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        }else{
            String result = adm.stock_update(id,name,stocks);
            adm.loadStockTable("", dtm);

            if (result == "stock updating succesfully") {
               succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "stock updating error") {
                 fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
