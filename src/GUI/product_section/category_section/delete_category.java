...
static Logger log=Logger.getLogger(delete_category.class.getName());
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        if(id.equals("") && name.equals("")){
            String result="cat deleted error";
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        }else{
            String result = adm.cat_delete(id);
            adm.loadCategoryTable("", dtm);

            if (result == "cat deleted succesfully") {
             succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "cat deleted error") {
              fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
