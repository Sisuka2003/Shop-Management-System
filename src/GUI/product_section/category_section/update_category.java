...
import static GUI.product_section.category_section.add_category.log;
...
static  Logger log=Logger.getLogger(update_category.class.getName());
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton6ActionPerformed
      String id = idField.getText();
        ...
        if(id.equals("") && name.equals("")){
            String result="cat update error";
               fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        }else{
            String result = adm.cat_update(id,name);
            adm.loadCategoryTable("", dtm);

            if (result == "cat updated succesfully") {
              succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "cat update error") {
              fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
