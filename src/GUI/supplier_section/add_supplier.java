...
public class add_supplier extends javax.swing.JFrame {
    ...
    static Logger log=Logger.getLogger(add_supplier.class.getName());
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        if (name.equals("") && company.equals("") && number.equals("")) {
            String result = "supplier adding error";
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        } else {
            String result = adm.supplier_reg(name, company, number);
            adm.loadSupplierTable("", dtm);

            if (result == "supplier added succesfully") {
                succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "supplier adding error") {
               fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
