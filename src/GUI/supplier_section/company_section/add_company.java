...
public class add_company extends javax.swing.JFrame {
    static Logger log=Logger.getLogger(add_company.class.getName());
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        String result="company adding error";
        fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        }else{
        String result = adm.company_reg(name, address, contact);
        adm.companyTableLoad("", dtm);

        if (result == "company added succesfully") {
            succese suceed = new succese(this,true);
            suceed.setVisible(true);
        } else if (result == "company adding error") {
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
