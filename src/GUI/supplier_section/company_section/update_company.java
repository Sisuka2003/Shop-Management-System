...
public class update_company extends javax.swing.JFrame {
    ...
    static Logger log = Logger.getLogger(update_company.class.getName());
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        if (name.equals("") && address.equals("") && contact.equals("")) {
            String result = "company updated error";
        fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        } else {
            String result = adm.company_update(id, name, address, contact);
            adm.companyTableLoad("", dtm);

            if (result == "company updated succesfully") {
               succese suceed = new succese(this,true);
            suceed.setVisible(true);
            } else if (result == "company updated error") {
                fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
