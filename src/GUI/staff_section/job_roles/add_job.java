...
public class add_job extends javax.swing.JFrame {
    static Logger log=Logger.getLogger(add_job.class.getName());
...
if(id.equals("") && name.equals("")){
           String result="job adding error";
           fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
}else{
        String result = adm.job_reg(name);
        adm.loadJobRoleTable("", dtm2);

        if (result == "job added succesfully") {
            succese suceed = new succese(this,true);
            suceed.setVisible(true);
        } else if (result == "job adding error") {
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
