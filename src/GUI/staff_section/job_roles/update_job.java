...
import static GUI.staff_section.job_roles.add_job.log;
 ...
 * @author DELL
 */
public class update_job extends javax.swing.JFrame {
    static Logger log=Logger.getLogger(update_job.class.getName());
  ...
  if(id.equals("") && name.equals("")){
           String result="job updating error";
           fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
  }else{
        String result = adm.job_update(id,name);
        adm.loadJobRoleTable("", dtm2);

        if (result == "job updated succesfully") {
            succese suceed = new succese(this,true);
            suceed.setVisible(true);
        } else if (result == "job updating error") {
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(update_job.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new update_job().setVisible(true);
            }
        });
    }
