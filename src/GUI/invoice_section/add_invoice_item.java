 ...
 * @author DELL
 */
public class add_invoice_item extends javax.swing.JFrame {

    static Logger log = Logger.getLogger(add_invoice_item.class.getName());
    DefaultTableModel dtm;
    DefaultTableModel dtm2;
    DefaultTableModel dtm3;
    invoiceModel adm;

    public add_invoice_item() {
        icon();
        initComponents();
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        String customer = invoIDField.getText();

        if (qty.equals("") && pid.equals("") && customer.equals("")) {
            String result = "invoice item added error";
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        } else {
            String result = adm.invoice_item_reg(qty, pid, customer);
            add_invoice ai = new add_invoice();
            adm.loadInvoiceTable("", ai.dtm);
            ai.setVisible(true);
            adm.loadInvoiceTable("", dtm2);
            adm.loadInvoiceItemTable(customer, dtm);
            adm.loadProductTable("", dtm3);

            if (result == "invoice item added successfully") {
                succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "invoice item added error") {
            fails f = new fails(this,true);
                f.popupstatus(result);
                f.setVisible(true);
                log.debug(result);
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        adm.loadProductTable(keyword, dtm3);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(add_invoice_item.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new add_invoice_item().setVisible(true);
            }
        });
    }
