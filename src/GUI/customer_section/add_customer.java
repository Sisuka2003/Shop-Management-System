...
public class add_customer extends javax.swing.JFrame {
    ...
    static Logger log = Logger.getLogger(add_customer.class.getName());
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        ...
        if (fname.equals("") && lname.equals("") &&mobile.equals("")) {
            String result = "customer added error";
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        } else {
            String result = adm.customer_reg(fname,lname,mobile);
            adm.loadCustomerTable("", dtm);

            if (result == "customer added successfully") {
                succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "customer added error") {
                fails f = new fails(this,true);
                f.popupstatus(result);
                f.setVisible(true);
                log.debug(result);
