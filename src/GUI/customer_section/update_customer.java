...
public class update_customer extends javax.swing.JFrame {
    ...
    static Logger log = Logger.getLogger(update_customer.class.getName());
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton5ActionPerformed
        ...
        if (id.equals("") && fname.equals("") && lname.equals("") && mobile.equals("")) {
            String result = "customer updated error";
            fails f = new fails(this,true);
            f.popupstatus(result);
            f.setVisible(true);
            log.debug(result);
        } else {
            String result = adm.customer_update(id, fname, lname, mobile);
            adm.loadCustomerTable("", dtm);

            if (result == "customer updated successfully") {
                succese suceed = new succese(this,true);
                suceed.setVisible(true);
            } else if (result == "customer updated error") {
                fails f = new fails(this,true);
                f.popupstatus(result);
                f.setVisible(true);
                log.debug(result);
